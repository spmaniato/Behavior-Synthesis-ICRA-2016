...
\todo[inline, caption = {Update ROS packages figure}]{(i) Add ReSpeC to image (move msgs to the left) (ii) Either number all steps or none (iii) Rename 3 services.}
}
...
Given the user's input (initial conditions and goals), the server first requests a full set of LTL formulas for \textsc{Atlas} from the $\mathtt{Generate LTL Specification}$ service ($\mathtt{vigir\_ltl\_specification}$ package).
...
The following excerpt\footnote{We have omitted some details for the sake of brevity and clarity of presentation. For example, most list elements are strings, e.g., \scriptsize{\texttt{"template\_id"}}.}
 is from the $\mathtt{State Instantiation}$ list message, which is the end product of the $\mathtt{vigir\_behavior\_synthesis}$ workflow (see Fig. \ref{Fig:vigir_behavior_synthesis}).
