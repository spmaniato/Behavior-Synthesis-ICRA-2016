...
Finally, the initial conditions we either be specified by the user or detected at runtime.
...
We can now automatically compile the task specification $\mathcal{T}_S$ in (the GR(1) fragment of) Linear Temporal Logic.
Since LTL is compositional, we can generate individual LTL formulas and then put them together to get the full LTL specification.
...
Formula \eqref{ActionOutcomeConstraintsFormula4} dictates that, if an outcome is $\False$ and the corresponding action is not activated, then that outcome should remain $\False$.
...
\end{equation}
where $\varphi_\mathcal{M}^{none} = \bigwedge_{m \in \mathcal{M}} \lnot \pi_m$ being $\True$ stands for not activating any control mode transitions.
