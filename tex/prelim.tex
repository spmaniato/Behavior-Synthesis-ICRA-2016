...
From this point on, we will refer to \textsc{Atlas} running Team ViGIR's software as the \emph{system}, $\mathcal{S}$.
...
Based on the FlexBE framework, developers create ``state implementations", $\mathsf{s} \in \mathsf{S}$. % Maybe definitions?
These are small, atomic blocks of code that each interface with some primitive, lower-level, system capability.
%, $\mathsf{p} \in \mathsf{P}$.
