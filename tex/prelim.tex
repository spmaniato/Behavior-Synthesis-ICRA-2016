...
Team ViGIR was awarded an Atlas (Fig. \ref{Fig:AtlasDoorFinals}), which is an anthropomorphic robot developed by Boston Dynamics, Inc. (BDI). 
We chose to leverage the basic capabilities provided by BDI's Application Programming Interface (API).
...
Team ViGIR has based its software on ROS.
...
The operator then detects objects of interest and overlays templates on top of them.
...
Team ViGIR's approach to high-level control is especially relevant to this paper.
Its corner stone is the Flexible Behavior Engine\footnote{\scriptsize{\url{https://github.com/team-vigir/flexbe_behavior_engine}}} (FlexBE \cite{Philipp2015MSc}), which is a major extension of the SMACH high-level executive \cite{SMACH2010RAM}.

Using the FlexBE framework, developers create ``state implementations", $Q$. % Maybe definitions?
Each $q \in Q$ is a small, atomic block of code that interfaces with \emph{one} of the lower-level system capabilities $\mathcal{C}$ (e.g., footstep planning).
...
Both composition of state machines and supervision of their execution takes place in FlexBE's graphical user interface\footnote{\scriptsize{\url{https://github.com/team-vigir/flexbe_chrome_app}}} (GUI).
